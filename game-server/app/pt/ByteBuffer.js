...
var ByteBuffer = function (org_buf, offset) {
    ...
    this.byte = function (val, index) {
        if (val == undefined || val == null) {
            if (_org_buf.length >= _offset + 1) {
                _list.push(_org_buf.readUInt8(_offset));
                _offset += 1;
            } else {
                _list.push(undefined);
            }
        } else {
    ...
    this.string = function (val, index) {
        if (val == undefined || val == null) {
            if (_org_buf.length < _offset + 2) {
                ...
                _offset += 2;
                if (_org_buf.length < _offset + len) {
                    _list.push(undefined);
                    _offset -= 2;
                    _org_buf['writeInt16' + _endian + 'E'](len, _offset);
                } else {
